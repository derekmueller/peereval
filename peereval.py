...
def readform(f):
    """
    Read an excel form in the current directory with positions of data hardcoded-in.
    ...
    team_eval: pandas dataframe
        contains the group, respondent, answers to q7 and comments on all team members
    ...
    grp_feedback = pd.DataFrame(
        {"group": [group], "respondent": [respondent], "feedback": [feedback]}
    )
    ...
    team_eval.insert(
        0, "respondent", value=respondent
    )  # add the respondent as a column
...
def dataValid(dfeval):
    """
    Check data to make sure it is valid, then run and look at output.
    ...
    dfeval[["q1", "q2", "q3", "q4", "q5", "q6", "q7"]] = dfeval[
    ...
    for grp in groups:
        ...
        for member in gmembers:
            ...
            m = (
                dfeval.loc[(dfeval["group"] == grp) & (dfeval["respondent"] == member)][
                    "member"
                ]
                .sort_values()
                .to_numpy()
            ...
            if not np.array_equal(gmembers, m):
                print(
                    "  ...Problem with form from {}, check that all group members are listed".format(
                        member
                    )
                )
            ### all data should be there
            d = dfeval.loc[
                (dfeval["group"] == grp) & (dfeval["respondent"] == member),
                ["q1", "q2", "q3", "q4", "q5", "q6", "q7"],
            ]
            if d.shape[0] != len(gmembers):
                print(
                    "  ...Problem with form from {}, check that all group members are evaluated".format(
                        member
                    )
                )
            if d.isnull().sum().sum() > 0:
                print(
                    "  ...Problem with form from {}, check that all criteria are evaluated".format(
                        member
                    )
                )
...
def calcPEM(dfeval, dfmember):
    """
    Calculate the Peer Evaluation Multiplier (PEM) for each group member.

    Note the dfeval will be modified.
    ...
    dfeval.insert(
        3,
        "score",
        dfeval[["q1", "q2", "q3", "q4", "q5", "q6", "q7"]].sum(axis=1).astype(float),
    )
    ...
    for grp in dfeval.groupby(["group"]).groups.keys():
        ...
        score_avg.loc[grp, ["pem"]] = pem.astype(float).round(4)
...
if __name__ == "__main__":
    ...
    description = (
        "Script to calculate Peer Evaluation Modifier from student evaluations"
    )
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument(
        "-d",
        "--dir",
        help="the directory where the xlsx files are found and the output csv files will be placed (defaults to current directory",
    )
    ...
    dfeval.to_csv("peereval.csv", index=False, float_format="%.4f", na_rep="NA")
    score_pem.to_csv("pem.csv", float_format="%.4f", na_rep="NA")
